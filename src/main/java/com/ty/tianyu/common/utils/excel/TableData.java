...
/**
 * 
 * @description 表格数据对象 Title: Excel导出工具<br>
 *              Description:
 *              封装了单个Sheet里除了表格标题及副标题等信息外的对象，包括表格表头对象、数据行对象集合及其操作方法等<br>
 * @author jiangxd
 * @modifier ty
 * @date 2014年4月1日 下午4:07:28
 */
public class TableData {
	...
	class SummaryData {
		...
		public String toString() {
			...
			sb.append(key).append("\t").append(index).append("\t")
					.append(count).append("\t");
	...
	private void buildSummary(int index) {
		...
		for (int j = 0; j < rows.size(); j++) {
			...
			if (okey != null && !okey.equals(key)) {
				for (int i = index; i < header.getColumnCount(); i++) {
					...
					if (thc.getAggregateRule() != null) {
						...
						} else if (thc.getAggregateRule().equalsIgnoreCase(
								"max")) {
			...
			for (int i = index + 1; i < header.getColumnCount(); i++) {
				TableColumn thc = header.getColumnAt(i);
				if (thc.getColumnType() != TableColumn.COLUMN_TYPE_FORMULA
						&& thc.getAggregateRule() != null) {
					Double value = Double.parseDouble(sd.row.getCellAt(i)
							.getValue());
